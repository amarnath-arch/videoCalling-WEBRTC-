...
export class RoomManager {
  ...
  private userToRooms: Map<WebSocket, string>;

  constructor() {
    this.rooms = new Map<string, Set<WebSocket>>();
    this.userToRooms = new Map<WebSocket, string>();
  }
  ...
  joinRoom(roomId: string, user: WebSocket) {
  ...
  getOtherUserInRoom(currentUser: WebSocket): WebSocket | undefined {
    ...
    const otherUser = [...(this.rooms.get(roomId ?? "") ?? [])].filter(
